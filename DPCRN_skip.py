...
class DPCRN_skip_model(Loss, Signal_Pro):
    ...
    def lossWrapper(self):
        ...
        def spectrum_loss_SD(s_hat, s, c=0.3, Lam=0.1):
            ...
            if self.loss_type == 'MIN':
                ...
                intra_update_rates = [self.skip_regular_MAE(gate, miu=self.target_rate) for gate in
                                      self.update_gates_intra]
                inter_update_rates = [self.skip_regular_MAE(gate, miu=self.target_rate) for gate in
    ...
    def enhancement(self, noisy_f, output_f='./enhance_s.wav', plot=True, gain=1, gamma=1, vad=None):
        ...
        enh_s, update_gate1_intra, update_gate2_intra, update_gate1_inter, update_gate2_inter = self.model_inference.predict(
            [np.array([noisy_s]) * gain, scale])
