...
class Trainer():
    ...
    def __init__(self, args):
        ...
        if self.config_dict['name'] == 'DPCRN-base':
            ...
            self.dpcrn_model = DPCRN_skip_model(batch_size=self.batch_size, length_in_s=self.second, lr=self.lr,
                                                config=self.config_dict)
            self.dpcrn_model.build_DPCRN_model()
        else:
            pass
        ...
        if self.mode == 'train':
            self.data_generator = data_generator(noise_dir=self.config_dict['database']['noise_dir'],
                                                 train_clean=self.config_dict['database']['train_clean'],
                                                 val_clean=self.config_dict['database']['val_clean'],
...
if __name__ == '__main__':
    ...
    parser.add_argument("--second", type=int, default=10, help='length in second of every sample')
    parser.add_argument("--ckpt", type=str,
                        default='./pretrained_weights/DPCRN_base/models_experiment_new_base_nomap_phasenloss_retrain_WSJmodel_84_0.022068.h5',
