...
class DprnnBlock_skip(keras.layers.Layer):
    ...
    def __init__(self, intra_hidden, inter_hidden, batch_size, L, width, channel, skip=0, **kwargs):
        ...
        if skip == 0:
            ...
            self.intra_rnn = keras.layers.Bidirectional(
                SkipGRU(units=intra_hidden // 2, return_sequences=True, return_state=True, implementation=1,
                        recurrent_activation='sigmoid', reset_after=False))
            self.intra_skip = 1
        else:
        ...
        if skip == 1:
            ...
            self.inter_rnn = SkipGRU(units=inter_hidden, return_sequences=True, implementation=1,
                                     recurrent_activation='sigmoid', reset_after=False)
            self.inter_skip = 1
        else:
            raise ValueError('the value of skip mode only support 0, 1, 2!')
    ...
    def call(self, x, scale):
        ...
        if self.intra_skip:
            ...
            update_gate_intra = tf.transpose(tf.concat([gate_forward[:, :, 0], gate_backward[:, :, 0]], axis=-1),
                                             [1, 0])
        else:
