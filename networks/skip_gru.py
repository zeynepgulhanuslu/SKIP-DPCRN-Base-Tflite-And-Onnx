...
class SkipGRUCell(Layer):
    ...
    def call(self, inputs, states, training=None):
        ...
        return [new_h, update_gate], [new_h, new_update_prob,
                                      new_cum_update_prob]  # tf.concat([h,new_update_prob,new_cum_update_prob],axis=-1)
...
class SkipGRU(RNN):
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        if initial_state is None:
            initial_state = [tf.zeros([tf.shape(inputs)[0], self.units]), tf.ones([tf.shape(inputs)[0], 1]),
                             tf.zeros([tf.shape(inputs)[0], 1])]
